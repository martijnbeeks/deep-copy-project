...
import { getInjectableTemplateForJob } from './job-results-helpers'

/**
 * Extract HTML templates from DeepCopy results
 * Centralized utility to avoid duplication
 */
export function extractHTMLTemplates(results: any): Array<{name: string, type: string, html: string, timestamp: string, angle?: string}> {
  ...
  try {
    ...
    if (results.swipe_results && Array.isArray(results.swipe_results)) {
      results.swipe_results.forEach((swipe: any, index: number) => {
        ...
        // Check for nested HTML in content field
    ...
    // Error extracting templates - return empty array
  }
 ...
 * Centralized utility to avoid duplication
 */
export async function storeJobResults(localJobId: string, result: any, deepCopyJobId: string) {
  try {
    // Extract HTML templates from the result
    const templates = extractHTMLTemplates(result)
    
    if (templates.length === 0) {
      // No templates found, but still store the result
    }
