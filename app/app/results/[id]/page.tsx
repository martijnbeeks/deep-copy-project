...
const DeepCopyResults = dynamic(
  () =>
    import("@/components/results/deepcopy-results").then(
      (mod) => mod.DeepCopyResults
    ),
  {
...
export default function ResultDetailPage({
  ...
  useEffect(() => {
    const fetchParentJob = async () => {
      if (currentJob?.parent_job_id) {
        try {
          const parent = (await internalApiClient.getMarketingAngle(
            currentJob.parent_job_id
          )) as any;
          setParentJob(parent);
          ...
          if (currentAvatarName && allAvatars.length > 0) {
            ...
            const currentIndex = allAvatars.findIndex(
              (a: any) => a.persona_name === currentAvatarName
            );
            const avatarNumber = currentIndex >= 0 ? currentIndex + 1 : null;

            // Count selected/researched avatars
            const selectedCount = allAvatars.filter(
              (a: any) => a.is_researched === true
            ).length;
        ...
        const selectedAvatars = allAvatars.filter(
          (a: any) => a.is_researched === true
        );
        setAvatarSequence({
          current: 1,
        ...
        <main className="flex-1 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-muted-foreground mb-4">
              The requested result could not be found or is not yet available.
            </p>
            <Link href="/dashboard">
  ...
  const selectedAvatars =
    currentJob?.avatars?.filter((a: any) => a.is_researched === true) || [];
  const selectedAvatarNames = selectedAvatars
    .map((a: any) => a.persona_name)
    .join(", ");
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-4 flex-wrap">
                          ...
                          {selectedAvatarNames && (
                            <>
                              ...
                              <span className="font-semibold">
                                {`${avatarSequence?.current} ) ${selectedAvatarNames}`}
                              </span>
                            </>
                          )}
                          <span>•</span>
                  ...
                  <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 border border-primary/20 text-primary">
                    <h1 className="text-sm font-medium">Research Results</h1>
                  </div>
                </div>
              </div>
            </div>
