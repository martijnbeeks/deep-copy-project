      ...
      for (const image of generatedImages) {
        ...
        if (imageStatus === 'success' && imageUrl) {
          ...
          if (isNew && storedAd) {
            ...
            // Record job credit event (one per image; idempotent by generated_static_ad id)
            if (organizationId && storedAd?.id) {
              try {
                ...
                await incrementOrganizationUsage(organizationId, 'static_ads') // analytics only
                logger.log(`💰 Recorded credit for new image: ${imageUrl}`)
              } catch (creditError: any) {
                logger.error(`⚠️ Failed to record credit: ${creditError.message}`)
              }
            }
          } else {
