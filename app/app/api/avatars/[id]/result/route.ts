...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (!id) {
      return createValidationErrorResponse('Avatar extraction job ID is required')
    }

    // Get access token using client credentials
    const accessToken = await getDeepCopyAccessToken()

    // Get avatar extraction job result with cache-busting
    const response = await fetch(`${DEEPCOPY_API_URL}avatars/${id}/result?t=${Date.now()}`, {
      method: 'GET',
      cache: 'no-store',
      headers: {
    ...
    if (!response.ok) {
      if (response.status === 404) {
        return createValidationErrorResponse('Avatar extraction result not available')
      }
