...
export function PromptEditor({ prompt, onSave, onCancel }: PromptEditorProps) {
  ...
  const requiredParams = prompt.required_params || []
  ...
  const updateDecorations = useCallback(() => {
    ...
    const markers: monacoEditor.IMarkerData[] = missing.map((p) => ({
              ...
              <div className="rounded-md bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 p-3">
                <div className="flex gap-2">
                  ...
                  <div className="text-xs text-red-800 dark:text-red-300">
                    <p className="font-medium">Cannot save: Missing required placeholders</p>
                    <p className="mt-1 text-red-700 dark:text-red-400">
                      Add the following to your prompt: {validation.missingRequired.map(p => `{${p}}`).join(", ")}
                    </p>
                  </div>
                </div>
              </div>
